...
- Live in-flight counter: the Task Completed button shows the number of in-progress tasks (pseudo-progress bar); a coral pill shows the running subagent count; flashes once when all tasks complete — no per-task noise.
- On Stop thinking indicator: an animated coral glyph on the On Stop button while Claude is processing (per-button opt-in via the Property Inspector).
